...
class RepositoryPolicyTest(unittest.TestCase):
    ...
    def test_step4e_write_only_mirror_receipt(self) -> None:
        receipt_path = ROOT / "audits/step4e-notion-write-receipt.json"
        if not receipt_path.exists():
            self.skipTest("Step-4E Notion mirror has not been emitted")
        receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
        page_map = json.loads((ROOT / "mirror/page_map.yaml").read_text(encoding="utf-8"))
        entry = next(
            item
