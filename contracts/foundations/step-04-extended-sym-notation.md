...
### 4.8 Ordered local-parameter convention

All odd local parameters are moved to the right.  For a displayed
component density \(\mathcal L\), first compute its symplectic
coefficient \(\mathcal C_{\mathsf s}^M\) and its constant-parameter
boundary coefficient \(\mathcal K_{\mathsf s}^M\), where
\(\mathsf s\) labels a supersymmetry generator:
...
=-\sum_\Xi\mathcal E_\Xi\mathcal R_{\mathsf s}^\Xi,}
\tag{4.42}
$$
