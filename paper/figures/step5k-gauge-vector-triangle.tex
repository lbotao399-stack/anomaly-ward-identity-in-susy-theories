...
\node[font=\bfseries] at (0,4.55) {$\displaystyle \text{gauge-vector triangle}$};
...
\draw[propagator] (L) -- (R)
  node[midway,above=3pt] {$\displaystyle u^C\!\leftrightarrow u^{C'},\quad r_1=k+q$};
...
\draw[propagator] (-5.25,2.75) -- (L)
  node[pos=.03,above right] {$\displaystyle \widetilde W_{c,\dot\alpha}^D(q)$};
\draw[propagator] (R) -- (5.25,2.75)
  node[pos=.97,above left] {$\displaystyle A_c^E(p)=D_+W_{c,+}^E(p)$};

\node[formula,anchor=east] at (-3.35,.15)
  {$\displaystyle \mathfrak V_{\widetilde W}=+\frac{ig}{4\hbar}c_{UCD}\widetilde W_c^D{}_{\dot\alpha}(\bar D_C^{\dot\alpha}-\bar D_U^{\dot\alpha})$};
\node[formula,anchor=west] at (3.35,.15)
  {$\displaystyle \mathfrak V_W=-\frac{ig}{4\hbar}c_{VC'E}W_c^{E\gamma}(D_{C'\gamma}-D_{V\gamma})$};

\node[formula,text width=13.6cm] at (0,-3.45)
  {$\displaystyle \mathcal I_{0,w}^{AB}=D_-[(Ku^A)e^{w\cdot\partial}(Ku^B)],\ K=-\frac{D_+\bar D^2D_+}{4\sqrt2}$\\[2pt]
   $\displaystyle D_0D_1D_2=r_{0,d}^2r_{1,d}^2r_{2,d}^2$};
\node[font=\ttfamily\footnotesize] at (0,-4.45) {G-WW-GAUGE-01};
\end{tikzpicture}
\end{document}
