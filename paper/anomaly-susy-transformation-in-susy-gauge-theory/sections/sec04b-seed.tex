...
the two rows being equal by relabeling of the Gaussian integral. With the
two labeled action-expansion assignments of paragraph~2 and their Taylor
factor, the Wick weight is
\begin{equation}\label{eq:ss:wick}
w_{\mathrm{Wick}}=\frac{1}{2!}(1+1)=1 ,
\end{equation}
the same arithmetic as the action-order factor
of~\eqref{eq:ss:seedactionorder}.
...
\end{equation}
The exponent Hessians are the second variations of $e^{-S/\h}$, i.e.
$\widehat H=-H/\h$ of~\eqref{eq:ss:seedhess} including the $1/\h$ of each
interaction vertex:
...
f^{UCD}f^{U'C'E}\,\delta_{UA}\delta_{U'B}\delta_{CC'}
\;\longrightarrow\;
f^{ACD}f^{BCE}
...
Equation~\eqref{eq:ss:seedassembly} is the canonical-frame bookkeeping:
vertex magnitudes $g/(4\h)$, propagators $-\h\,\delta^{AB}p^{-2}$, and the
explicit insertion factor $1/64$ of~\eqref{eq:ss:seedinsertfactor}. In the
unit-insertion frame used for the remaining steps, the same graph weight is
organized with the ordered cubic vertex factors
...
frames are related by the rescaling of~\eqref{eq:ss:rules-framecomp}: each of
the two vertices gains $(\sqrt2)^{2}$, each of the two insertions gains
$\sqrt2$ (the product $1/64\to1/32$, the $1/32$ appearing
in~\eqref{eq:ss:dweight}), and each of the three internal lines loses $1/2$,
a net factor $1$. Each cubic vertex carries $\h^{-1}$ and each of the three
propagators carries $\h$, leaving the net one-loop power $\h$ in either
frame. The signed endpoint-row weight of~\eqref{eq:ss:seedsigntable} is a
property of the closed triangle word and applies to either completion route.
\end{remark}
...
\end{equation}
The four derivative endpoint rows of the closed (parent) triangle word, with
their raw and transport signs, are
\begin{align}
(r_0,r_1):&\quad (-1)(-1)=+1,\notag\\
...
anomaly. With $D_0=\ell^{2}$, $D_1=(\ell+q)^{2}$, $D_2=(\ell+p+q)^{2}$, the
...
momentum projection. Like the endpoint-row signs of
Remark~\ref{rem:ss:seedsigns}, this per-row chain is a recorded primitive of
the underlying ledger, independently machine-verified row by row. There is no
extra edge factor $2$ and no metric-trace
