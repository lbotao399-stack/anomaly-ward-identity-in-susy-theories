...
\begin{tikzpicture}[scale=1.15]
  ...
  \draw[sleg]  (ML) -- ++(180:1.5) node[left]  {$\partial_{\dot\alpha}c\ (q)$};
  \draw[sPhit] (MR) -- ++(0:1.5)   node[right] {$\widetilde\Phi^{r}\ (p)$};
...
source-$\gamma$ reversed-chiral line $r_{2}$, with $r_{1}-r_{0}=p$. The external legs are the antichiral letter
$\gamma^{r}=\widetilde\Phi^{r}$ (momentum $p$) and the vector output letter $\partial_{\dot\alpha}c$ (momentum
$q$). The gauge--matter topology $T_{GM}^{(r)}$ replaces the left matter vertex by the cubic gauge vertex $G$; its
six ordered routings of $(r_{0},r_{1},r_{2})$ are the six permutation routes of the text, and the reversed channel
$(\gamma^{r},b)$ has the same figure with the source attachments exchanged.}
...
\mathcal F^{\mathrm{DRED}}_{b>\gamma} +\mathcal C^{(d)}_{0}+\mathcal C^{(d)}_{1} =8\mul^{2}\big(\mathcal J_{1}-\mathcal R_{0}\big) =-1024\,\mul^{2}\,\mathfrak b(p) ,
\end{equation}
collapsing to $(-1024\,p_{\dot2},\,+1024\,p_{\dot1})$ on the two dotted spinor components (the sign convention of
...
\end{equation}
in units of $(\h g^{2}/16\pi^{2})\,f^{ACD}f^{BCE}$: the two-matter orbit contributes $-1$ on $e_{1}$ and the
gauge--matter orbit $+1$ on $e_{2}$. Equivalently, in the unit normalization $X=\nabla_{+}\cW_{+}$,
...
intermediate lines, and Figure~\ref{fig:bg:triangle} is drawn from the recorded edge routings.
\end{remark}
