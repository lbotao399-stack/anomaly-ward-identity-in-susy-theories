...
\draw[sleg] (v1) -- ++(210:1.25) node[pos=0.52,sinsert]{}
  node[pos=1,below left=0pt] {$\nabla_{+}\Phi_{r}$};
\draw[sleg] (v2) -- ++(330:1.25) node[pos=1,below right=0pt] {$\widetilde\Phi^{s}$};
\draw[sV] (v0) -- ++(90:1.25) node[pos=1,above=1pt] {$\cV$};
\node at (3.1,0) {$=\;0$};
\end{tikzpicture}
...
ordered sources are the marked insertion $\nabla_{+}\Phi_{r}$ (filled node)
and the antichiral leg $\widetilde\Phi^{s}$; the external field-strength
letters enter through the preimages
$\bcW_{\dot\alpha}^{(1)}=-\tfrac18\cD^{2}\bcD_{\dot\alpha}\cV$ on the vector
leg. Every ordinary parent of the family vanishes on the physical output word
$\langle\partial c,\partial c\rangle$, as displayed; the kernels themselves
are nonzero (Remark~\ref{rem:ss:bc:control}). The anomaly is instead the
regulated coincident Euler Jacobian of the kinetic
...
Section~\ref{sec:superspace:bb} (equation~\eqref{eq:ss:bb-projectorid}) is
displayed there with the local convention $\det r=-\bar r^{\,2}$, while the
half square in~\eqref{eq:ss:bc:marked} is displayed as
$8\,\bar r_{e}^{2}\,\cD^{2}\delta_{e}$. The two source audits record this
$D$-algebra square in two sign conventions for the determinant factor; each
convention is used consistently within its own subsection, the physical
coefficients of both channels were machine-verified, and no result is
affected.
\end{remark}
