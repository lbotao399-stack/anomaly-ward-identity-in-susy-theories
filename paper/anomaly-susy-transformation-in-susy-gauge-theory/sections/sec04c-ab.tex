...
\end{scope}
\end{tikzpicture}
\caption{The ordered triangles of the $(b,\beta_r)/(\beta_r,b)$ family in the routing~\eqref{eq:ss:ab:routing}: one $\nabla_+\cW_+$ and one $\nabla_+\Phi_r$ insertion (filled dots), residual external legs $\partial c(p)$, $\beta_r(q)$. G3 shares the G2 line content with one matter-cubic vertex replaced by the antichiral superpotential vertex.}
...
=\frac{W(p,q)}{96\pi^{2}},
\end{equation}
from $2\int_{\Sigma_2}x_i\,dx_1dx_2=\tfrac13$ per coordinate. Applied graph by graph: G1 integrates to the mark coefficients $(\tfrac43,\tfrac23)$, $(\tfrac23,\tfrac43)$ in the two slot orderings, i.e.\ $2X_{\partial c\,\beta}+2E$, a pure total derivative ($2X+2E=2\,P^{\dot\alpha}(\beta_r\,\partial_{\dot\alpha}c)$) vanishing in the quotient; G2 integrates to the selected word $\tfrac23\,\beta_r(2p+q)^{\dot\alpha}\partial_{\dot\alpha}c$ plus the transported word $-\beta_r\,q^{\dot\alpha}\partial_{\dot\alpha}c$, i.e.\ $\tfrac13\,\beta_r(4p-q)^{\dot\alpha}\partial_{\dot\alpha}c$ with pair/equation-of-motion coefficients $(-\tfrac13,\tfrac43)$; G3 carries the marked simplex weights $\tfrac23,\tfrac13$ per mark and sums to the raw wedge coefficients $+2\sqrt2\,i$ ($G_{3,2}$) and $-2\sqrt2\,i$ ($G_{3,3}$) in units of $\h g^{2}/(16\pi^{2})$. Passing from the raw momentum wedge to the dotted bilinear flips the sign once, since with the locked phase $e^{ipx}$, $(ip)_+\wedge(iq)_+=-(p_+\wedge q_+)$.
...
\end{equation}
with $\varepsilon_{rst}=+1$ and $E$ the equation-of-motion composite of step 3, entering in both orderings. The total-derivative combinations are $T_{\partial c\,\beta}=X_{\partial c\,\beta}+E =P^{\dot\alpha}(\beta_r\,\partial_{\dot\alpha}c)$ and $T_{\beta\,\partial c}=-X_{\beta\,\partial c}+E$, so $aX_{\partial c\,\beta}+bE=(a-b)X_{\partial c\,\beta}+bT_{\partial c\,\beta}$ and $aX_{\beta\,\partial c}+bE=(a+b)X_{\beta\,\partial c}+bT_{\beta\,\partial c}$. The coefficient flow of the family is the aligned display
\begin{align}
...
\end{align}
The shift is fixed by residual supersymmetry: acting with the $q_r$ of~\eqref{eq:ss:qaction} (stated with the channel table in Section~\ref{sec:superspace:table}) on a general vector $x=(x_1,x_2,x_3,x_4)$ in the four-component basis, the three Ward conditions $q_s[\,x\cdot(\text{basis})\,]=0$ read
...
\paragraph{8. The final result.} Restoring the letter normalizations~\eqref{eq:ss:letters} --- the insertions contribute $(-i/\sqrt2)(1/\sqrt2)=-i/2$ and each dotted output pair contributes $i/\sqrt2$ --- the renormalized vector~\eqref{eq:ss:ab:flowren} becomes the channel-table row
