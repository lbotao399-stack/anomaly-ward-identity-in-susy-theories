...
- `STEP5K_STRICT_SUPERGRAPH_COMPLETION`: gauge and adjoint-matter triangles now have separate typed graph IR and TeX-rendered PDF/SVG figures.  The canonical gauge pre-integral coefficient is
...
not $\hbar g^2/2$; the isolated directed seed is $-\lambda_1/8$.  The two factors $2$ are the same-edge anticommutator normalizations, while the endpoint sums give $L_1=r_0+r_1$ and $L_2=r_1+r_2$.  The 48-row generator proves the local $D$-word/contact stream only; the full $\lambda_1(DA-AD)$ gauge term remains sourced to the separate accepted 269-check external-slot reconstruction.
